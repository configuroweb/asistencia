	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'students' => array('Estudiantes', '', 'resources/table_icons/group.png', 'None'),
			'units' => array('Unidades', '', 'resources/table_icons/books.png', 'None'),
			'courses' => array('Cursos', '', 'resources/table_icons/attributes_display.png', 'None'),
			'attendance' => array('Registro de Asistencia', '', 'resources/table_icons/application_view_icons.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'students' => "`students`.`regno` as 'regno', `students`.`name` as 'name', IF(    CHAR_LENGTH(`courses1`.`name`), CONCAT_WS('',   `courses1`.`name`), '') as 'course'",
			'units' => "`units`.`id` as 'id', `units`.`name` as 'name'",
			'courses' => "`courses`.`id` as 'id', `courses`.`name` as 'name'",
			'attendance' => "IF(    CHAR_LENGTH(`students1`.`name`), CONCAT_WS('',   `students1`.`name`), '') as 'student', IF(    CHAR_LENGTH(`students1`.`regno`), CONCAT_WS('',   `students1`.`regno`), '') as 'regno', `attendance`.`week` as 'week', if(`attendance`.`date`,date_format(`attendance`.`date`,'%m/%d/%Y'),'') as 'date', IF(    CHAR_LENGTH(`units1`.`name`), CONCAT_WS('',   `units1`.`name`), '') as 'unit', `attendance`.`attended` as 'attended', `attendance`.`id` as 'id'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'students' => "`students` LEFT JOIN `courses` as courses1 ON `courses1`.`id`=`students`.`course` ",
			'units' => "`units` ",
			'courses' => "`courses` ",
			'attendance' => "`attendance` LEFT JOIN `students` as students1 ON `students1`.`regno`=`attendance`.`student` LEFT JOIN `units` as units1 ON `units1`.`id`=`attendance`.`unit` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Attendance_Management_System_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Attendance_Management_System_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Attendance_Management_System_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Attendance_Management_System_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Control de Asistencia ConfiguroWeb</a>
			</div>
