...
function units_insert(){
	...
	sql('insert into `units` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function units_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `attendance` where `unit`='".addslashes($id[0])."'", $eo);
...
function units_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `units` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function units_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Detalles de Unidad', $templateCode);
