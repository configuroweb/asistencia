...
function courses_insert(){
	...
	sql('insert into `courses` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function courses_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$res = sql("select `id` from `courses` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `students` where `course`='".addslashes($id[0])."'", $eo);
...
function courses_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `courses` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function courses_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Detalles de Curso', $templateCode);
